...
export const useSnakeGame = create<SnakeGameState>()(
  subscribeWithSelector((set, get) => ({
    ...
    updateSnakePits: (deltaTime: number) => {
      ...
      // Helper function to check if a rattlesnake has completed its patrol cycle
      const hasCompletedPatrolCycle = (snake: any) => {
        if (!snake.patrolPoints || snake.patrolPoints.length === 0) return true;
        
        // A full patrol cycle is complete when:
        // 1. Snake has visited all points and returned to start (currentPatrolIndex === 0 and patrolDirection === 1)
        // 2. Or snake has completed a forward-backward cycle (has been to end and back to start)
        if (!snake.patrolCycleStarted) {
          // Mark cycle as started when snake begins moving
          return false;
        }
        
        // Check if snake has completed a full forward-backward cycle
        // This happens when currentPatrolIndex returns to 0 with direction 1 after having moved
        return snake.currentPatrolIndex === 0 && snake.patrolDirection === 1 && snake.hasReachedEnd;
      };
      ...
      updatedSnakes.forEach((snake, snakeIndex) => {
        if (
          ...
          // Initialize patrol cycle tracking if not present
          if (snake.patrolCycleStarted === undefined) {
            updatedSnakes[snakeIndex] = {
              ...snake,
              patrolCycleStarted: true,
              hasReachedEnd: false
            };
          }
          
          // Track when snake reaches the end of patrol points
          if (snake.currentPatrolIndex >= snake.patrolPoints.length - 1 && !snake.hasReachedEnd) {
            updatedSnakes[snakeIndex] = {
              ...updatedSnakes[snakeIndex],
              hasReachedEnd: true
            };
          ...
          switch (currentState) {
            case "patrolling":
              ...
              if (snake.isLightEmergence) {
                  ...
                  if (
                    ...
                    if (snake.isChasing) {
                      ...
                      updatedSnakes[snakeIndex] = {
                        ...
                        rattlesnakePauseStartTime: currentTime,
                        isChasing: false,
                      };
                    }
                  }
                ...
                // Normal emergence: patrol until full cycle complete - let normal AI handle movement/detection
                ...
                if (isPitLitForNormalPatrol) {
                  // Pit is lit, continue patrolling indefinitely regardless of patrol completion
                  // Don't check patrol completion, just keep patrolling
                } else {
                  // Pit is not lit, check if patrol cycle is complete
                  if (hasCompletedPatrolCycle(snake)) {
                    // If currently chasing player, switch to chasing state for another full cycle
                    if (snake.isChasing) {
            ...
            case "chasing":
              ...
              if (isPitLit) {
                ...
                // No patrol completion check while pit is lit
              } else {
                // Pit is not lit, chase until patrol cycle complete then return
                if (hasCompletedPatrolCycle(snake)) {
                ...
                } else if (!snake.patrolCycleStarted) {
                  // Just started chasing after pit stopped being lit, initialize patrol tracking
                  updatedSnakes[snakeIndex] = {
                    ...snake,
                    patrolCycleStarted: true,
                    hasReachedEnd: false,
            ...
            case "pausing":
              // Pause for 200ms
              if (
                snake.rattlesnakePauseStartTime &&
                currentTime - snake.rattlesnakePauseStartTime >= 200
              ) {
                ...
                if (snake.pitPosition) {
                  ...
                  if (distance <= 10) {
                    ...
                    updatedSnakes[snakeIndex] = {
                      ...
                      rattlesnakePauseStartTime: undefined,
