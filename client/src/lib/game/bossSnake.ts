...
function checkBoulderCollision(
  snake: Snake,
  newPosition: Position,
  boulders: Boulder[],
): Boulder | null {
...
function getWallCollisionInfo(
  snake: Snake,
  newPosition: Position,
  walls: Wall[],
): { hit: boolean; wall?: Wall; normal?: { x: number; y: number } } {
  const snakeRect = {
    x: newPosition.x,
    y: newPosition.y,
    width: snake.size.width,
  ...
  for (const wall of walls) {
    if (checkAABBCollision(snakeRect, wall)) {
      ...
      const overlapX = Math.min(
        snakeRect.x + snakeRect.width - wall.x,
        wall.x + wall.width - snakeRect.x,
      );
      const overlapY = Math.min(
        snakeRect.y + snakeRect.height - wall.y,
        wall.y + wall.height - snakeRect.y,
      );
...
function clampToBounds(
  position: Position,
  snakeSize: { width: number; height: number },
...
function getPatrolTarget(snake: Snake): Position {
  ...
  const index = Math.max(
    0,
    Math.min(snake.currentPatrolIndex, snake.patrolPoints.length - 1),
  );
...
function calculateChargeRampedSpeed(snake: Snake, currentTime: number): number {
  if (
    !snake.chargeRampStartTime ||
    !snake.chargeBaseSpeed ||
    !snake.chargeMaxSpeed
  ) {
...
export function updateBossSnake(
  snake: Snake,
  walls: Wall[],
  dt: number,
  player?: Player,
  ...
  switch (snake.bossState) {
    case "tracking":
      ...
      if (snake.playerSnapshot) {
        ...
        snake.direction = getDirectionVector(
          valerieCenter,
          snake.playerSnapshot,
        );
      }
    ...
    case "charging":
      ...
      if (snake.direction && snake.playerSnapshot) {
        const currentChargeSpeed = calculateChargeRampedSpeed(
          snake,
          currentTime,
        );
        ...
        const hitBoulder = boulders
          ? checkBoulderCollision(snake, newPosition, boulders)
          : null;
        if (hitBoulder && !snake.currentChargeHitBoulder) {
          ...
          if (
            hitBoulder.lastHitTime &&
            currentTime - hitBoulder.lastHitTime < 120
          ) {
          ...
          if (hitBoulder.hitCount >= hitBoulder.maxHits) {
            ...
            const destroyedCount =
              boulders?.filter((b) => b.isDestroyed).length || 0;

            // Only spawn photophobic snake after 1st and 3rd boulder destruction
            const shouldSpawnPhotophobic =
              destroyedCount === 1 || destroyedCount === 3;
            ...
            const currentlyDestroyedCount =
              boulders?.filter((b) => b.isDestroyed).length || 0;
            const totalBouldersHit =
              boulders?.filter((b) => b.hitCount > 0).length || 0;

            // Only spawn on first hit of boulder if it's the 1st or 2nd boulder to be hit
            const shouldSpawnScreensaverSnake =
              !hitBoulder.hasSpawnedScreensaver &&
              hitBoulder.hitCount === 1 &&
              totalBouldersHit <= 2;
          ...
          recoilDistance = Math.max(
            minRecoil,
            Math.min(maxRecoil, recoilDistance),
          );
          ...
          while (
            recoilDistance > minRecoil &&
            (checkWallCollision(snake, recoilTargetPosition, walls) ||
              (boulders &&
                checkBoulderCollision(snake, recoilTargetPosition, boulders)))
          ) {
            recoilDistance *= 0.8;
          ...
          if (levelBounds) {
            recoilTargetPosition = clampToBounds(
              recoilTargetPosition,
              snake.size,
              levelBounds,
            );
    ...
    case "recoiling":
      ...
      if (
        snake.recoilStartTime &&
        snake.recoilTargetPosition &&
    ...
    case "projectileBarrage":
      ...
      if (
        snake.projectileBarrageStartTime &&
        ...
        if (
          currentTime - snake.projectileBarrageStartTime >=
          totalBarrageDuration
        ) {
    ...
    case "waitingForPhantom":
      ...
      if (
        snake.phantomSpawnStartTime &&
        snake.phantomSpawnCount !== undefined &&
        snake.phantomSpawnCount < maxPhantoms
      ) {
    ...
    case "snakeRain":
      ...
      if (
        snake.snakeRainStartTime &&
        snake.snakeRainCount !== undefined &&
        snake.snakeRainCount < maxRainSnakes
      ) {
      ...
      if (
        ...
        if (
          ...
          currentTime - snake.snakeRainStartTime >= totalRainDuration
        ) {
