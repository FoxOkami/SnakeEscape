...
import {
  useKeyBindings,
  type KeyBindings,
} from "../../lib/stores/useKeyBindings";
import {
  drawStandardTooltip,
...
const HubRoom: React.FC = () => {
  ...
  const [editingKeyBinding, setEditingKeyBinding] = useState<string | null>(
    null,
  );
  ...
  const handleCheatCode = () => {
    ...
    if (cheatCodeInput.replace(/\s+/g, "").toLowerCase() === "tangential") {
      ...
      // Immediately activate the item to apply shield health
      useInventoryItem(shinyobjectItem.id);
      ...
      if (currentEditingKey) {
        ...
        if (e.code !== "Escape") {
          ...
          const conflictingAction = Object.entries(currentKeyBindings).find(
            ([action, keyCode]) =>
              keyCode === e.code && action !== currentEditingKey,
          );

          if (conflictingAction) {
            setKeyBindingError(
              "Unable to bind the same key to multiple actions",
            );
  ...
  useEffect(() => {
    ...
    const gameLoop = (currentTime: number) => {
      ...
      if (interactionState === "conversation") {
        ...
        ctx.font = "24px Arial";
        ctx.textAlign = "center";
        ctx.fillText("Would you like to play?", canvas.width / 2, dialogY + 60);
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    >
                      {editingKeyBinding === "up"
                        ? ""
                        : getKeyDisplayText(keyBindings.up)}
                    </button>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    >
                      {editingKeyBinding === "down"
                        ? ""
                        : getKeyDisplayText(keyBindings.down)}
                    </button>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    >
                      {editingKeyBinding === "left"
                        ? ""
                        : getKeyDisplayText(keyBindings.left)}
                    </button>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    >
                      {editingKeyBinding === "right"
                        ? ""
                        : getKeyDisplayText(keyBindings.right)}
                    </button>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    >
                      {editingKeyBinding === "interact"
                        ? ""
                        : getKeyDisplayText(keyBindings.interact)}
                    </button>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    >
                      {editingKeyBinding === "secondaryInteract"
                        ? ""
                        : getKeyDisplayText(keyBindings.secondaryInteract)}
                    </button>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    >
                      {editingKeyBinding === "walking"
                        ? ""
                        : getKeyDisplayText(keyBindings.walking)}
                    </button>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    >
                      {editingKeyBinding === "dash"
                        ? ""
                        : getKeyDisplayText(keyBindings.dash)}
                    </button>
                  </div>
                </div>
      ...
      <div className="absolute top-4 left-20 z-50 flex gap-2">
        ...
        {(() => {
          ...
          return (
            <Badge
              className={`${canDash ? "bg-blue-600" : "bg-gray-600"} text-white`}
            >
